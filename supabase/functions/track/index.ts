...
function parseUserAgent(ua: string, useUAParser: boolean = true): ParsedUserAgent {
  if (!useUAParser) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!site) {
      return new Response(
        JSON.stringify({ error: 'Invalid tracking ID or inactive site' }),
        {
          status: 404,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    if (event_type === 'link_click') {
      if (!link_url || !link_type) {
        return new Response(
          JSON.stringify({ error: 'Missing link click data' }),
          {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          }
    ...
    if (!page_url) {
      return new Response(
        JSON.stringify({ error: 'Missing page_url' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    const { data: existingSession } = await supabase
      .from('sessions')
      .select('id, first_seen, page_count, entry_page')
